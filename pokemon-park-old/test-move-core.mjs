...
check('hold parks the machine in impact until it is released', () => {
  const m = createPhaseMachine({ travelSpeed: 10, impactTime: 0.2, fadeTime: 0.2, easeIn: 0, hold: true, maxHold: 0 });
  m.spawn(makeLine({ from: { x: 0, z: 0 }, to: { x: 1, z: 0 } }));
  for (let i = 0; i < 300; i++) m.update(1 / 60);
  assert(m.phase === Phase.IMPACT && m.holding && m.alive, 'did not hold: ' + m.phase);
  m.release();
  assert(!m.holding, 'still holding after release');
  for (let i = 0; i < 30; i++) m.update(1 / 60);
  assert(m.phase === Phase.DONE, 'did not finish after release: ' + m.phase);
});

check('maxHold releases a hold nobody let go of, and respawn re-arms it', () => {
  const m = createPhaseMachine({ travelSpeed: 10, impactTime: 0.2, fadeTime: 0.2, easeIn: 0, hold: true, maxHold: 1 });
  const line = makeLine({ from: { x: 0, z: 0 }, to: { x: 1, z: 0 } });
  m.spawn(line);
  for (let i = 0; i < 40; i++) m.update(1 / 60);
  assert(m.holding, 'released too early');
  for (let i = 0; i < 120; i++) m.update(1 / 60);
  assert(m.phase === Phase.DONE, 'maxHold did not fire: ' + m.phase);
  m.spawn(line);
  for (let i = 0; i < 40; i++) m.update(1 / 60); // well inside maxHold again
  assert(m.holding, 'respawn did not re-arm the hold');
});
