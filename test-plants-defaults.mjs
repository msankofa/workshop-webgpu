...
const SPECIES_COUNT = 4; // the 4 launch herbs; the 2 sprigClump shrubs were cut 2026-08-08
const keys = Object.keys(PLANT_PRESETS);
ok(keys.length === SPECIES_COUNT, `exactly ${SPECIES_COUNT} presets`);
ok(keys.includes('chickweed') && keys.includes('cleavers') && keys.includes('mint') && keys.includes('jewelweed'), 'the 4 named herb species');
// sprigClump renders as bare opaque quads without the alpha-cutout texture it needs, so no
// shipped preset may use it until that texture exists.
ok(Object.values(PLANT_PRESETS).every(p => p.leaf.style !== 'sprigClump'), 'no shipped preset uses sprigClump');
