// Node checks for structure-viewer-v2.html (the merge of structure-viewer.html and the spawn-atrium viewer). Two halves:
...
// Run: node test-structure-viewer-v2.mjs
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { generateOne, generateStructures, STRUCTURE_DEFAULTS } from './bot-structures.js';
import { ECO_KINDS, generateEco } from './base-game-spawn-layout.js';

const here = path.dirname(fileURLToPath(import.meta.url));
const src = fs.readFileSync(path.join(here, 'structure-viewer-v2.html'), 'utf8');
...
ok(group && !['maze', 'bots', 'ui', 'structures'].includes(group[1]),
  `the preset slot group is distinct from bot-viewer-v3's and v1's (got '${group && group[1]}')`);
...
const seedExpr = /const slotSeed = \(base, i\) => \(base \+ i \* (\d+) \+ \(slotSeedOffsets\[i\] \|\| 0\) \* (\d+)\)/.exec(src);
...
console.log('\neco-brutalist family');

// Every eco kind the generator exposes must be selectable, and the gallery must place all of them.
const ecoLabels = /const ECO_LABELS = \{([\s\S]*?)\};/.exec(src)?.[1] || '';
ok(ECO_KINDS.every(k => ecoLabels.includes(`${k}:`)), 'every eco kind has a dropdown label',
  ECO_KINDS.filter(k => !ecoLabels.includes(`${k}:`)).join(', '));
ok(/generateEco\(s\.kind, ecoParams, seed/.test(src), 'the eco generator is called with the live eco params and a per-slot seed');

// Eco floor slabs span -0.3..0. The flat ground and the terrain pad under a specimen must sit at
// the slab base, or the slab either floats or is buried.
ok(/const GROUND_TOP = -0\.3/.test(src), 'flat eco ground sits at the floor-slab base');
ok(/out\.pads\.push\(\{[^}]*y:\s*GROUND_TOP/.test(src), 'an eco specimen levels its terrain pad at the same height');

// The complex is the widest eco specimen; the gallery cell has to hold every room without overlap.
const ECO_CELL = Number(/const ECO_CELL = (\d+)/.exec(src)?.[1]);
ok(Number.isFinite(ECO_CELL), `ECO_CELL is declared (${ECO_CELL})`);
let ecoWorst = { kind: null, radius: 0 };
for (const kind of ECO_KINDS) {
  if (kind === 'complex') continue;
  for (let seed = 1; seed <= 50; seed++) {
    const r = generateEco(kind, {}, seed).radius;
    if (r > ecoWorst.radius) ecoWorst = { kind, radius: r };
  }
}
console.log(`  widest eco room over 50 seeds: ${ecoWorst.kind} at ${ecoWorst.radius.toFixed(1)} m radius`);
ok(ecoWorst.radius * 2 <= ECO_CELL, `the eco gallery cell holds the widest room (${(ecoWorst.radius * 2).toFixed(1)} m across vs ${ECO_CELL} m cell)`);

// Occlusion needs the concrete re-marked after every rebuild, because clearBoxes disposes the meshes.
ok(/occluders:\s*mapRoot/.test(src), 'flora is created with the concrete as its occluder set');
ok(/flora\.markOccluders\(mapRoot\)/.test(rebuildBody), 'every rebuild re-marks the occluders');
