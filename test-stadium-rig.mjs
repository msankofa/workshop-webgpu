...
check('every shipped species walks, whatever the mapper made of it', () => {
  ...
  const shipped = fs.readdirSync('models/stadium').filter(f => f.endsWith('.glb'));
  assert(shipped.length >= 12, `only ${shipped.length} models are checked in`);
...
const ALL_SPECIES = fs.readdirSync('models/stadium').filter(f => f.endsWith('.glb')).map(f => f.replace('.glb', ''));
