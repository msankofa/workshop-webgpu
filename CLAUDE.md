...
| Vegetation | `vegetation.md` | `trees.js`, `forest-gpu.js`, `forest-placement.js`, `grass.js`, `grass-compute.js`, `grass-look.js`, `flora-occlusion.js`, `tree-viewer.html`, `tree-lod-preview.js` |
...
| Roads | `roads.md` | `roads.js`, `road-network.js`, `road-index.js`, `road-mesh.js`, `road-path.js`, `road-projection.js`, `road-projection-worker.js`, `trail-router.js`, `base-game-plan.js`, `base-game-sites.js`, `base-game-trails.js` (wired into `bot-viewer-v3.html` and `base-game.html`) |
...
| Materials (TSL demos) | `materials.md` | `materials/index.js`, `materials/material-demo-api.js`, `materials/dissolve.js`, `materials/hologram-visor.js`, `materials/damage-overheat.js`, `materials/foliage-sss.js`, `material-viewer.html`, `concrete-material.js` (cast concrete graph shared by the bot viewer and the Base Game spawn building; documented in `bots.md`) |
| Infra / debug UI | `infra.md` | `frame-profiler.js`, `gpu-pipeline-meter.js`, `render-pass-recorder.js`, `environment-ui.js`, `world-map.js`, `disk-store.js`, `tools/filesystem-map.html` (WebGPU/TSL holographic 3D force-directed node-link map of the repo filesystem, filterable by type/date, served by `serve.py`'s `/api/fs-scan`) |
...
| Flight | `flight.md` | `flight-model.js`, `flight-airframes.js`, `flight-terrain.js`, `flight-ai.js`, `flight-combat.js`, `flight-drones.js`, `flight-autopilot.js`, `flight-meshes.js`, `flight-hud.js`, `water-hybrid.js`, `demos/flight-sim.html` (blast debris via `blast-debris-sim.js`/`blast-debris.js`) |
...
| Pokémon Lab | `pokemon-lab.md` | `pokemon-lab.html` (the page; browse, segments, picking, posing and annotating shipped), `pokemon-lab-runtime.js` (the seam a game reads the lab through: static JSON in, glTF node ids out, no THREE and no server), `pokemon-rig.js` (facts about a skeleton), `pokemon-annotation.js` (what a person decided each part is), `pokemon-lab-io.js` (the one file, plus the dex list), `pokemon-pose.js` (how far apart two poses are; pure, not wired in), `pokemon-select.js` (picking bones and chains), `pokemon-ik.js` (FABRIK, and which bones answer a drag), `pokemon-hang.js` (a ragdoll built from a rig, stepped by `ragdoll.js`, whose cone solver gained an optional `min` and per-cone `stiffness` — both default to its old behaviour), `pokemon-drive.js` (what moves each bone: the clip, nothing, or the ragdoll — the mask behind the partial ragdoll); v1 plan in `docs/pokemon-lab/v1-plan.md`, the maths and its limits in `docs/pokemon-lab/math.md`. Replaces the stadium-walker line for new work — v1 is browse/annotate/pose only, v2 is movement, v3 is moves |
