...
| Infra / debug UI | `infra.md` | `frame-profiler.js`, `gpu-pipeline-meter.js`, `environment-ui.js`, `world-map.js`, `disk-store.js`, `tools/filesystem-map.html` (WebGPU/TSL holographic 3D force-directed node-link map of the repo filesystem, filterable by type/date, served by `serve.py`'s `/api/fs-scan`) |
