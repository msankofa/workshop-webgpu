...
export function createBaseGameDroneView({ scene, worldCoordinates, tintFor = () => 0x8ea2b8, cameraObstruction = null } = {}) {
  ...
  function disposeMesh(mesh) {
    if (!mesh) return;
    scene.remove(mesh);
    // Geometry is this craft's; materials are shared and reference counted by flight-meshes.js, where
    // dispose() releases one reference and frees only when the last craft using it is gone.
