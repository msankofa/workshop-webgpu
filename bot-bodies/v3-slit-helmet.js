// bot-bodies/v3-slit-helmet.js — FROZEN HISTORICAL BODY: v3 — slit helmet (2026-07-31 11:26)
//
// Adds the wraparound visor helmet. Snapshot taken before the Mark VIII helmet pass.
//
...
const HELMET_MARK_VII = [
  ...
  // ONE continuous low crest, centred, front-to-back. (Previously a bar plus two stray spikes at
  // different heights and materials, which read as antenna clutter rather than a crest.)
  { anchor: 'head', type: 'rbox', role: 'accent', position: [0, 0.126, -0.010], size: [0.034, 0.026, 0.240], bevel: 0.006 },

  // THE VISOR: a SLIM horizontal wraparound band with a FLAT bottom edge, ending at cheek height.
  // The previous one ran brow-to-chin and came to a point — a beak, not a visor — and a bright
  // strip across its middle split it back into the two-lens read it was meant to replace.
  { anchor: 'head', type: 'extrude', role: 'rubber', smooth: false, position: [0, 0.026, 0.100], depth: 0.052, bevel: 0.005, seg: 1,
    outline: [[-0.100, -0.024], [-0.106, 0.008], [-0.078, 0.032], [0.078, 0.032], [0.106, 0.008], [0.100, -0.024]] },
  // glow sits ON the visor's lower lip (its boundary), not stranded across its middle
  { anchor: 'head', type: 'rbox', role: 'eye', position: [0, 0.004, 0.150], size: [0.176, 0.012, 0.014], bevel: 0.003 },

  // brow ridge HOODS over the top edge of the visor
  { anchor: 'head', type: 'extrude', role: 'metal', smooth: false, position: [0, 0.058, 0.096], depth: 0.066, bevel: 0.005, seg: 1,
    outline: [[-0.112, -0.006], [-0.088, 0.030], [0.088, 0.030], [0.112, -0.006], [0.088, -0.020], [-0.088, -0.020]] },
  // CHEEK RAILS flanking the visor in a third material, connecting brow to mandible
  { anchor: 'head', type: 'rbox', role: 'metal', position: [0.086, -0.012, 0.086], size: [0.030, 0.090, 0.090], rotation: [0, 0.30, 0], bevel: 0.005 },
  { anchor: 'head', type: 'rbox', role: 'metal', position: [-0.086, -0.012, 0.086], size: [0.030, 0.090, 0.090], rotation: [0, -0.30, 0], bevel: 0.005 },

  // MANDIBLE: a distinct blunt volume in SHELL (not visor-black), shallow — no forward point.
  { anchor: 'head', type: 'rbox', role: 'shell', position: [0, -0.076, 0.070], size: [0.150, 0.078, 0.120], bevel: 0.012 },
  { anchor: 'head', type: 'rbox', role: 'plate', position: [0, -0.104, 0.104], size: [0.100, 0.040, 0.052], bevel: 0.008 },
  { anchor: 'head', type: 'rbox', role: 'rubber', position: [0, -0.082, 0.132], size: [0.070, 0.036, 0.020], bevel: 0.004 },

  // FLUSH side plates: hugging the 0.118 shell, not standing off it.
  { anchor: 'head', type: 'rbox', role: 'plate', position: [0.100, -0.006, -0.012], size: [0.026, 0.104, 0.126], rotation: [0, 0, 0.08], bevel: 0.006 },
  { anchor: 'head', type: 'rbox', role: 'plate', position: [-0.100, -0.006, -0.012], size: [0.026, 0.104, 0.126], rotation: [0, 0, -0.08], bevel: 0.006 },
  { anchor: 'head', type: 'rbox', role: 'metal', position: [0.108, -0.006, -0.012], size: [0.012, 0.062, 0.084], bevel: 0.004 },
  { anchor: 'head', type: 'rbox', role: 'metal', position: [-0.108, -0.006, -0.012], size: [0.012, 0.062, 0.084], bevel: 0.004 },

  // rear vents, low profile, no spikes
  { anchor: 'head', type: 'rbox', role: 'metal', position: [0.046, 0.046, -0.114], size: [0.018, 0.052, 0.052], rotation: [0.25, 0, 0], bevel: 0.004 },
  { anchor: 'head', type: 'rbox', role: 'metal', position: [-0.046, 0.046, -0.114], size: [0.018, 0.052, 0.052], rotation: [0.25, 0, 0], bevel: 0.004 },
];
