...
const cfg = (over) => ({ ...AIM_BLEND_DEFAULTS, ...over });
...
// ── the split ────────────────────────────────────────────────────────────────────────────────
{
  const out = solveAimBlend(0.4, 0.2, AIM_BLEND_DEFAULTS, {});
...
}
{
  ...
  const l = solveAimBlend(-0.4, -0.2, AIM_BLEND_DEFAULTS, {});
  const r = solveAimBlend(0.4, 0.2, AIM_BLEND_DEFAULTS, {});
...
}
{
  ...
  const out = solveAimBlend(3.0, 1.4, AIM_BLEND_DEFAULTS, {});
...
{
  const out = solveAimBlend(Math.PI * 2 + 0.3, 0, AIM_BLEND_DEFAULTS, {});
  near(out.torsoYaw, 0.3 * AIM_BLEND_DEFAULTS.torsoYawShare, 1e-9, 'residual is wrapped before it is split');
}
{
  const out = solveAimBlend(NaN, undefined, AIM_BLEND_DEFAULTS, {});
...
// ── channel stepping ─────────────────────────────────────────────────────────────────────────
{
  ...
  const solved = solveAimBlend(0.4, 0.2, AIM_BLEND_DEFAULTS, {});
  for (let i = 0; i < 600; i++) stepAimChannels(ch, solved, AIM_BLEND_DEFAULTS, 1 / 60, 1);
  ...
  for (let i = 0; i < 600; i++) stepAimChannels(ch, idle, AIM_BLEND_DEFAULTS, 1 / 60, 0);
...
}
{
  ...
  const ch = newAimChannels();
  const solved = solveAimBlend(3.0, 0, AIM_BLEND_DEFAULTS, {});
  stepAimChannels(ch, solved, AIM_BLEND_DEFAULTS, 1 / 60, 1);
...
near(barrelTrimFraction(0, AIM_BLEND_DEFAULTS, 1 / 60), 1, 1e-12, 'a zero error needs no limiting');
{
  const dt = 1 / 60, err = 1.0;
  near(barrelTrimFraction(err, AIM_BLEND_DEFAULTS, dt), (AIM_BLEND_DEFAULTS.barrelRate * dt) / err, 1e-12,
    'the fraction is rate*dt/error');
  ok(barrelTrimFraction(0.001, AIM_BLEND_DEFAULTS, dt) === 1, 'a correction inside the frame budget is applied whole');
  ok(releaseTrimFraction(1.0, AIM_BLEND_DEFAULTS, dt) < barrelTrimFraction(1.0, AIM_BLEND_DEFAULTS, dt),
...
}
{
  ...
  for (let i = 0; i < 240; i++) {
    ...
    const out = solveAimBlend(bearing - bodyYaw, 0, AIM_BLEND_DEFAULTS, {});
...
// ── lead (D) ─────────────────────────────────────────────────────────────────────────────────
near(aimLeadSeconds(50, 0, AIM_BLEND_DEFAULTS), 0, 1e-12, 'hitscan gets no lead at the shipped defaults');
near(aimLeadSeconds(50, Infinity, AIM_BLEND_DEFAULTS), 0, 1e-12, 'an infinite speed gets no lead');
near(aimLeadSeconds(54, 108, AIM_BLEND_DEFAULTS), 0.5, 1e-12, 'a projectile leads by its flight time');
near(aimLeadSeconds(500, 108, AIM_BLEND_DEFAULTS), AIM_BLEND_DEFAULTS.leadMaxS, 1e-12, 'lead is capped');
...
near(aimLeadSeconds(0, 100, cfg({ leadLatencyS: 0.12 })), 0.12, 1e-12, 'latency lead is available but off by default');

// ── recoil (D) ───────────────────────────────────────────────────────────────────────────────
{
  let r = 0;
  r = addRecoil(r, AIM_BLEND_DEFAULTS);
  ...
  for (let i = 0; i < 300; i++) r = stepRecoil(r, AIM_BLEND_DEFAULTS, 1 / 60);
  near(r, 0, 1e-6, 'recoil settles back to zero');
  near(addRecoil(0, cfg({ recoilEnabled: false })), 0, 1e-12, 'recoil off');
  near(stepRecoil(0, AIM_BLEND_DEFAULTS, 1 / 60), 0, 1e-12, 'stepping a settled recoil is a no-op');
}
