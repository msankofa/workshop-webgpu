...
- `export async function applyTerrainTextures(root, mapData, meta = {}, options = {})` — returns `{ materials, texturedMeshes, report }` or `null` (no `document`, or missing resolution/world size). **Blending is per-triangle, not continuous:** `classifyMesh` assigns one dominant material per vertex (argmax of biome/mask/slope), then buckets each triangle by majority vote into a `geometry.addGroup` range bound to a separate `MeshStandardMaterial` — so adjacent differently-classified triangles meet at a hard seam with no cross-fade. Replacing this with continuous per-vertex splat-weight blending is the subject of `research/terrain-appearance-analysis/` (see `MERGED-TERRAIN-UNDERSTORY-PLAN.md`).
