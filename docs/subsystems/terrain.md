...
| (volume LOD cascade) | Not a module: `base-game-terrain.js` runs extra `terrain-system.js` instances with `params.lod > 0`, `segmentsPerChunk` fixed and `volumetric: true`; in Base Game every system shares ONE `terrain-worker-pool.js` pool (`options.workerPool`; default cores/2−1 threads, at most 4) and only a system without one spawns its own `params.workerCount` (0 = min(4, cores−2)) workers behind a round-robin `worker` facade; the v5 source builds `volume` (and `normals`) at any lod with `createDensityPoint(..., spacing)` band-limiting warp/cave noise (`createUnboundedDensityNoiseSampler().fbm3(..., octaves, bandSpacing)`). See `base-game.md`. |
| `terrain-splat-streamed.js` | Ground textures for the STREAMED terrain (Base Game chunks + volume cascade). Per-fragment layer weights from world height and slope — sand/grass/dirt/rock/snow (`splatWeights(h, ny)` CPU twin; rock triplanar, the rest planar xz) — over ordinary mipmapped textures from `textures/ground/<folder>/{color,normal}.jpg` (`loadStreamedSplatTextures({ slots })`; a project's `material.slots` maps each layer to a folder, `splatSlotFolders(slots)` fills the gaps with the layer name, and the returned set records `slots`; `placeholderStreamedSplatTextures` for Node). `replaceStreamedSplatImages(current, next)` swaps the pictures behind a bound set in place (same `THREE.Texture` objects, new `image`) and returns an `{ averages }` patch for `updateStreamedSplat`, so a project change re-textures every instance and the grass ground node without a rebuild; `base-game-terrain.js` exposes it as `terrain.swapSplatTextures(next)` / `terrain.splatSlots`. `splatConfigFromProject(project)` (pure) turns a project's cfg beach/rock/snow rules into this material's frame (gradient slope → `normal.y = 1/sqrt(1+g²)`, shore top mid-way up the beach band) with `material.rules` (the same uniform names) on top; the Base Game applies the patch through `terrain.updateSplat` on project apply and once the textures land. **Per-biome overrides** (`material.biomes`): `biomeLayerTable(material)` (pure) lists the distinct override folders (array layer order, max `SPLAT_MAX_BIOME_LAYERS` = 16) and a `SPLAT_MAX_BIOMES` (32) × 5 `Int32Array` of layer indices (-1 = the project-wide slot); `biomeRuleTable(project)` (pure) gives each biome its seven thresholds (own rules over the project-wide ones; `biomeRuleCfg(rules, b)` turns a row into a cfg for `splatWeights`). `loadStreamedSplatBiomeArray(entries, { size })` (browser) packs the folders into two mipmapped `DataArrayTexture`s (1024² RGBA per layer; r184's WebGPU backend generates mips per layer) with per-layer averages; `placeholderStreamedSplatBiomeArray` for Node. `createStreamedSplatMaterial(textures, overrides, { biome: { idNode } })` binds the tables as `uniformArray`s and reads the biome id under the fragment through `idNode(xz)` (nearest texel, -1 where no field has streamed): each slot's weight uses that biome's rule row, and its sample comes from the array layer the table names (`texture(arr, uv).depth(layer).grad(...)`) or the plain slot texture when -1. `updateStreamedSplat` accepts `biomeTable`, `biomeRules`, `biomeAverages`. `base-game-terrain.js`: `terrain.setSplatBiomes({ textures, table, rules } | null)` holds a placement-field-window handle while bound, builds `idNode` from `fieldWindow().gpuSampler('biomeIds')` plus a render-origin uniform (synced on rebase), rebuilds the per-LOD instances when the window or the array changes and patches the tables in place otherwise. The grass ground-colour node still samples the five slot textures, so blades in an overridden biome tint toward the project-wide colour, not the biome's. Built so it cannot turn to static at range: a 7× far tiling takes over from 40–220 m, and from `fadeNear`–`fadeFar` (250–1400 m) the albedo settles on each layer's average colour (its 1×1 mip, measured from the image) while normal strength and the sampler-free macro hash fade to zero (`detailFade(d)` twin). `createStreamedSplatMaterial(textures, overrides)` → `MeshStandardNodeMaterial`. Reads are the cost, so each layer is sampled only where its weight > 0.004 (`If` per layer): the uv derivatives are hoisted to vars in uniform flow (an anchor pins their declaration before the branches) and every sample uses `.grad()`, which is what WGSL requires inside non-uniform control flow — flat grass costs one layer. One `Fn` on `colorNode` does all sampling (three evaluates colour → roughness → normal) and passes roughness and the object-space tilted normal to the other two nodes through `property()`s; the normal goes through `transformNormalToView`; `updateStreamedSplat(mat, { tileMeters, fadeFar, … })` live; `{ lod: { self, finer } }` binds two `terrain-lod-coverage.js` maps for the LOD dissolve (`syncStreamedSplatCoverage(mat)` after a recentre); `{ water }` binds the Base Game water's `groundShade` uniforms for the wet tide band and the Snell caustic emissive. Tests: `test-terrain-splat-streamed.mjs` (headless build via `tsl-build-check.mjs`). An optional `rain` bundle `{ uniforms, offset, puddleScale, rippleScale }` beside `water`, behind `If(uWetness > 0)` so a dry world skips its 315 lines of noise, wets the whole ground (albedo down, roughness down, puddles on the flats, ripple normals) using the fields imported from `rain.js` — one copy of that maths, not a twin — gated to what is out of the water by the same `submerged` term the tide band uses. |
...
- **`killPlaneYAt(x, z)` is cached per player column** (2026-09-09). Base Game calls it every frame for the player's XZ, and in volumetric mode `groundHeight` is a full `surfaceYAt` density scan of that column (`terrain-generator-js.js` `hashedCell3`/`fbm3`/`densityAt` were 4-5% of allocation weight in a walking heap profile). `groundHeight` is a pure function of `(system.source, system.epoch, volumetricMode)`, so the answer is recomputed only when one of those changed or the query moved more than `KILL_PLANE_CACHE_M` (0.5 m) in x or z. The plane sits `killPlaneBelowSurface` (80 m) under the ground, so half a metre of horizontal staleness cannot change the test except in the one frame a player crosses a cliff edge, and the next recompute half a metre later catches it. `killPlaneYAtExact(x, z)` is the uncached form, for tests only; `test-base-game-terrain.mjs` asserts the cache equals it at every recompute point and that a `setSource` swap invalidates it.
- **The per-frame batch lists are reused** (2026-09-09). `syncBatchVisibility` collected its removals with `[...batched.keys()]` — a fresh array of every batched key, on every batcher, every frame; it now collects only the keys it will actually remove, into one reused array (still collected before deleting, because a Map must not be mutated mid-iteration). `batchTargets()` rebuilt its list and one record per streamer two or three times a frame (`applyMaterialsPass` twice plus `integrate`); it now fills one reused array of reused records. This is safe only because no caller keeps the reference past its own scope and no caller nests a second `batchTargets()` inside an iteration of the first — check that before adding a caller. `stats.residency` is deliberately NOT reused: `stats` records are copied and kept by the performance capture.
...
- `maxCompactionsPerFrame` (default 0 = unlimited, opt-in) in `terrain-chunk-batches.js` rations
  `BatchedMesh.optimize()`. Measured on r0.184 (`three.core.js:26574-26663`): `optimize()` shifts
  only the geometries after the earliest gap and adds **one update range per shifted geometry**, so
  the worst case (the first entry evicted from a full batch) is the whole tail and the best case
  (only trailing entries evicted) is free — zero ranges, no upload. The ration stays because the
  CPU `copyWithin` and that tail case are both real, not because every compaction is a full rewrite.
  A batcher using it must call `beginFrame()` each frame or the ration never refills.

### Batch upload accounting (logical, not submitted)

`terrain-chunk-batches.js` counts what it marks dirty, separated by kind. Every byte is **logical**:
the size of the `BufferAttribute` update ranges this code appended, measured by diffing
`updateRanges` around each mutation. The WebGPU backend issues one `writeBuffer` per range and does
not coalesce (`WebGPUAttributeUtils.updateAttribute`, `three.webgpu.js:77684-77765`), so submitted
bytes are at least these — the submitted side is counted by the renderer instrumentation, not here.

| Field | Meaning |
|---|---|
| `firstUploads` / `firstUploadBytes` | A chunk copied into a batch for the first time: one `addGeometry`, whose ranges cover its own vertices and indices. Equals the geometry's own byte size. |
| `visibilityFlips` | `setVisibleAt` calls that actually changed a value. No attribute is dirtied, but each one stops `onBeforeRender` early-outing and forces a whole-indirect-texture upload, so it is the module's one ordinary per-frame mutation. |
| `fallbacks` | A chunk that fit no batch and kept drawing its own mesh. Not an upload; the counterpart to a first upload. |
| `compactions` / `compactionShifts` / `compactionBytes` | `optimize()` calls, the number of geometries that actually moved (one position range each), and the bytes those ranges cover. |

There is **no in-place mutation of batch buffers**: a re-tint (`colorizeGeometry`) writes a fresh
`color` attribute on the *chunk's own* geometry and reaches a batch only as a remove + re-add, which
shows up as a first upload. Its own cost is reported separately as `colorizeBytes`.

`batcher.takeFrameUpload()` reads and clears the frame slice; it is deliberately **not** part of
`beginFrame()`, which the page calls twice per frame (two material passes) and which only refills
the compaction ration. `batcher.stats.upload` is the cumulative total.

`base-game-terrain.js` sums both across the near batcher and every cascade batcher:

- `frameCost.batchFirstUploads` / `batchFirstUploadBytes` / `batchVisibilityFlips` / `batchFallbacks`
...
(`world.terrain.runtime = terrain.stats`, `terrainCost = { ...terrain.frameCost }`) — no page edit.

### Batch lifecycle

`test-terrain-chunk-batches.mjs` [5] and `test-base-game-terrain.mjs` `[batch lifecycle]` exercise
this in Node: 150 evict/refill cycles reuse `BatchedMesh` geometry and instance slots (measured:
`_geometryInfo.length` and `_instanceInfo.length` stay at their initial size, so `deleteGeometry` /
`deleteInstance` free slots rather than leak them); `clear()` empties every map and the group and the
batcher is reusable afterwards; `dispose()` detaches the group. Over 900 m of travel the batched set
tracks residency exactly and the batch list never grows past what is in use. A `setSource` swap
reuses the same chunk keys, and `add()`'s `if (byKey.has(key)) remove(key)` plus
`syncBatchVisibility`'s removal of keys the streamer no longer holds leave no stale entry and no
orphan visible instance. What Node cannot show: the actual `writeBuffer` traffic, GPU memory, and
whether the driver coalesces anything.
...
posted job is counted until its reply is handled on the main thread, so a stalled main thread can
hold a finished job in the count for up to a frame, and a worker descheduled mid-job still counts;
it is not a thread-CPU measurement, and every reply carries `jobMs`, the worker-clock duration of that job. Jobs are stamped
...
system and the three cascade levels, reports `stats.workers`, and disposes it last.

Why: the DevTools trace of 2026-09-07 showed four systems × min(4, cores−2) = 16 terrain threads
...
home, detach vs dispose, unavailable pool, a system given a pool spawns nothing). The scheduler,
coverage and tint tests' fake workers echo `owner` like the real one.
