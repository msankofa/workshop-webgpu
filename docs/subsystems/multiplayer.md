...
| `server/publish-map.js` | Pure validation/merge helpers (`validateSegment`, `validateSecret`, `mergeMapConfig`, `validateProjectArtifact`) plus `publishMap`'s GitHub Git Data API orchestration (GLB, `-data.json`, `map-config.json` and, when the client sends `projectJson`+`projectHash`, a byte-exact `-project.json`) and `handlePublishRequest`'s HTTP glue | ~215 |
