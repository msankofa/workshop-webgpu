...
The environment viewer exposes these values under **Weapon control → Muzzle / firing FX**. Edits update the selected weapon immediately and are included in named slider-state presets and the `pcw:weaponTuning` localStorage record, so they survive a browser refresh. The source defaults remain in `weapons.js`; browser tuning does not rewrite source files.

At fire time, the muzzle marker's world quaternion rotates its local +Z axis into the smoke/flash direction. While the local first-person effect remains alive, the environment viewer refreshes its source position and direction from the live muzzle pose every render frame; strafing, camera motion, bob, and recoil therefore cannot leave the viewmodel effect behind. The ballistic hit ray still begins at the authoritative camera/head origin, but the visual tracer captures the muzzle's world position before recoil and uses that fixed point as its start. This lets the line converge from the barrel to the camera-aimed hit without following the player after firing. Networked muzzle points are accepted only when they remain within two world units of the validated shot origin; bots, third-person shots, and missing anchors retain the near-eye fallback. Tracers and impacts otherwise remain fixed in world space. The effect renderer scatters smoke in a barrel-relative basis and applies vertical rise separately, instead of adding the old world-X/Z jitter. Smoke starts at the muzzle by default and moves forward according to the selected weapon's profile.
...
### 9. Remote players do not use this IK path

Remote multiplayer representations currently use simplified body/hand placeholders rather than the local procedural body plus real weapon-anchor arm IK. This contract therefore describes the local third-person body, not remote player hand placement.
