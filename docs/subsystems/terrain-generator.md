...
| File | Responsibility |
|---|---|
| `terrain-generator-v5.html` | Node-graph canvas UI (forked from v4): panels for noise fields, classic composer, **layer stack**, erosion/hydrology, derived masks, biome rules, material masks (plus **ground texture slots**: one select per runtime layer sand/grass/dirt/rock/snow, folders from `serve.py`'s `/api/list-ground-textures`, saved as the project's optional `material: { version: 1, slots }` block and read by the Base Game; the material canvas, its legend and the 3D preview's material colouring use each chosen texture's average colour (`materialRgbaFromMasks(masks, colorTable, out)` in `terrain-generator-js.js`, the same function `buildMaterialMasks` uses with `MATERIAL_COLORS`; forest is the grass slot darkened since the game draws forest with the grass texture). A second block of checkbox+slider rows edits `material.rules` (`shoreTop`, `grassTop`, `dirtTop`, `snowBottom`, `snowTop`, `rockSlope`, `rockFull`, the splat's own uniform names): unchecked rows show the value the game derives from the World group's cfg and stay out of the file. Below them, **per-biome overrides**: all 18 biomes listed, each behind a checkbox that opens its card with five pickers (project-wide fallback shown in the default option) and its own rule rows, saved as `material.biomes[name] = { slots?, rules? }`; the material canvas colours a cell from its biome's table when that biome has its own slots (`materialRgbaFromMasks`'s `perBiome` argument). The material canvas does not yet honour the height rules; it still uses the studio's own biome/slope masks — `docs/terrain-studio-ground-texture-plan.md`), **paint**, **import**, **3D preview**, **export & project**, real-map viewer, reference tables. Owns the worker request queue, history, paint stroke handling, exports. |
