...
| File | Responsibility |
|---|---|
| `terrain-generator-v5.html` | Node-graph canvas UI (forked from v4): panels for noise fields, classic composer, **layer stack**, erosion/hydrology, derived masks, biome rules, material masks (plus **ground texture slots**: one select per runtime layer sand/grass/dirt/rock/snow, folders from `serve.py`'s `/api/list-ground-textures`, saved as the project's optional `material: { version: 1, slots }` block and read by the Base Game — `docs/terrain-studio-ground-texture-plan.md`), **paint**, **import**, **3D preview**, **export & project**, real-map viewer, reference tables. Owns the worker request queue, history, paint stroke handling, exports. |
