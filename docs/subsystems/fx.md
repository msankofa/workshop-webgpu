...
`base-game.md` "The weapon flashlight". Its laser sight, `weapon-laser.js`, is the
same choice taken further: the DOT is a SpotLight with a sub-degree cone and `decay: 0` (a collimated
beam does not spread, so it does not dim), which means no raycast and correct occlusion for free.
Only the beam in the air is drawn, because no light source can express one, and depth testing clips
it where it meets geometry. Note the base game does **not** use v3's
