...
`passPostMs` is one timer around `renderer.render`, so it can say the encode is expensive but not
which phase is. This attributes it:
...
| `three` (capture context) | Which Three the page ran: `vendor/three-0.184` with `REVISION` and `sharedLightUniforms`. Base Game serves its own copy of the 0.184 build from `vendor/three-0.184/` (importmap for `three`, `three/webgpu`, `three/tsl`; addons and three-mesh-bvh still come from the CDN and resolve `three` through the map) with one local patch, applied by `scratchpads/fps-churn/patch-three-shared-lights.py` to a fresh copy of `node_modules/three/build/three.webgpu.js`: light and shadow uniform nodes are cached per light (`lightSharedUniform`, `sharedRenderReference`), so instanced meshes of one material share one render bind group and camera and light uniforms are written once a frame, not once per object. `setSharedLightUniforms(false)` / `?sharedlights=0` restores upstream behaviour. `test-three-shared-light-uniforms.mjs` proves the node identity across two instanced builds, flag on and off, and covers distinct lights not sharing, property mutation, a shadow-casting spot, two material graphs, disposal and recreation, and the flag turned off after nodes exist (15 checks) | Re-apply the patch script after any Three upgrade; the root `three.webgpu.js` is an unpatched grep copy |
| `uniformWriteRows` | For every `updateBinding` of a uniforms group, the changed uniforms named `group/uniform:ValueType @object/material` (a range starts at the changed uniform's offset; the node's own name in parentheses when the author set one; the object and material being encoded from the `_renderObjectDirect` wrapper), tallied per frame and per scene, top 16 by count (`UNIFORM_ROWS`). Added 2026-09-09: the allocation profiles put three's per-changed-uniform range object and Map entry first among allocators standing still, and this says which uniform changes on every object every frame | A uniform in the per-object group that is the same for every object (sun direction, time) is rewritten once per object; moving it to the render group (as the grass wind was) writes it once per frame |
| `bindingWrites` / `bindingWriteBytes` | `backend.updateBinding` calls, and the bytes those calls write: the sum of the binding's update ranges times the buffer's element size (`WebGPUBindingUtils.updateBinding` issues one `writeBuffer` per range), or the whole buffer when there are none. Read before the call, because `Bindings._update` clears the ranges after the upload | Calls are the writes three decided to submit (a uniforms group whose diff found a change), not dirty ranges; bytes follow the backend's own rule |
...
| `pipelineCreates` / `pipelineCreateMs` / `pipelineCreateRows`, `programCreates` / `programCreateMs` | `Pipelines._getRenderPipeline` calls (a render-pipeline cache miss: the one place a render object's shader is compiled) with the stage clock around each, and up to 8 rows naming the object, material type and `material.name`, the time, and whether the async path (`promises` non-null) was taken; `backend.createProgram` calls (shader modules) counted and timed the same way | Exact as call counts; the time is wall time around the call, which for the sync path includes the browser's compile and for the async path only the request. Sits inside `pipelinesMs`, never added to it. A frame with a long `pipelineCreateMs` compiled a shader; a long frame without one did not, whatever `pipelinesBuilt` says |

Limits of the byte counters, from the backend source: a storage attribute with three components is padded to four by the backend (`_force3to4BytesAlignment`), which the counter cannot see, so its bytes read 3/4 of what was written; an interleaved attribute keeps its ranges on the shared `InterleavedBuffer`, which the counter does not consult, so it reads the whole backing array; the first upload of an attribute or binding goes through `createAttribute`/`createBindings`, not these counters; and a counted call is one hook call, which the backend may turn into several `writeBuffer` calls (one per range). The counts are therefore the backend's own arithmetic over what our code marked dirty, not a measurement of queue traffic.
...
`terrainInFlight` (added 2026-09-07 after `heapMB`) is the number of terrain worker jobs outstanding at frame time (queued at a worker or running); `terrainBusyWorkers` beside it is the number of terrain workers holding unacknowledged work at that moment (posted, reply not yet handled); with jobs of 0.25-1.7 s and frames of tens of ms it approximates the threads executing, but a stalled main thread inflates it by at most one frame's worth of replies, so it is not a thread-CPU measurement. The worker-count A/B across captures was confounded by route and session drift, and outstanding jobs alone cannot distinguish a backlog from concurrency.
