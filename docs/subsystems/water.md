...
- `base-game-water-sim.js` (2026-08-23) is the physics half of that sea and the only water the simulation reads: `createBaseGameWaterSim({ level, waves, enabled })` builds a `buildWaveTable` from the room's wave keys and answers `heightAt(x, z, t)` (null when disabled). No three.js, no terrain — the page and `server/base-game-rooms.js` both import it so the client's prediction and the authoritative simulation swim in the same swell. It does not model the render's `shallowFade`.
- `base-game-water.js` (2026-08-23) is Base Game's sea: `createOceanSurface` at the terrain's sea level over the streamed `terrain-sea-depth.js` window, reflecting `sky.colorAlong`, lit by the rig's sun, wave spectrum from the room's shared keys, visibility gated on the depth window's minimum ground height; `surfaceHeightAt(x, z)` is the CPU twin for cameras/bodies; `groundShade` is the uniform bundle the ground material's wet band + caustics run on (`terrain.setSplatWater`). Panel-facing look setters (2026-08-26): `setRefraction(on)` (a uniform mixing `bedColorAt` between the shallow colour and the refracted framebuffer, so no rebuild), `setFoam(scale)` scaling all four foam strengths from an authored base, `setClarity(depthScale)`, `applyPreset(name)` and `refreshLookBase()` (re-read the base after `applyWaterConfig` writes the profile). For it `createOceanSurface` gained optional hooks: `worldOffset` (vec2 added to scene xz → global xz so waves/depth survive a render-origin rebase), `reflection(viewDir, N, thickness)`, `bedColorAt(viewDir, N, thickness)`, `thicknessAt(vertexThickness)`, `shallowFade` (metres of depth over which the displacement ramps from flat; the window depth is read before displacement for it), `foamFade` ([start, end] m over which foam energy fades out; `makeSurfaceShading` takes it as `farFade`) — all default to the previous behaviour. See `base-game.md`. Its three `viewportDepthTexture` reads pass an explicit `FloatType` `DepthTexture` (2026-09-07): with `reversedDepthBuffer` the framebuffer depth is depth32float, canvas or scene pass alike, and three's shared default target is depth24plus, so the per-frame copy was refused with a console error and the water read stale depth.
