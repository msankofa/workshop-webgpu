...
compiles and a two-minute `render warmup`. The offset is now a `uniform(offset, 'uint')`, the WGSL
is identical across variants and only one program per mesh role compiles (`test-forest-gpu-programs.mjs`
builds the WGSL headlessly through `WebGPUBackend.createNodeBuilder` and asserts it). Pipelines are
still one per material per pass, but a pipeline over a cached program is cheap. WebGPU creates a pipeline per material per pass, lazily, on the
...
| `test-forest-gpu-programs.mjs` | `forest-gpu.js` material graphs | Builds every variant mesh's WGSL with a stub renderer and asserts one distinct program per mesh role across variants (9 roles, 9 programs for 4 variants); a per-variant constant in the graph fails it. |
