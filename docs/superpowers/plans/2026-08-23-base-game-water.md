...
STATUS 2026-08-23: W1 shipped (sea level descriptor field + protocol + facade + server + wave keys; `test-base-game-sea-level.mjs`); W2 shipped (`terrain-sea-depth.js` on the clipmap window, wired into the facade; `test-terrain-sea-depth.mjs`). W3 shipped (`base-game-water.js` surface over the depth map, sky `colorAlong`, hybrid hooks; `test-base-game-water.mjs`). W4 shipped (per-pixel depth-buffer thickness, framebuffer refraction, shallow-water damping; DOF-pass interaction unverified). W5 shipped (sky/planar/ssr reflection modes with the throttled reflector). W6 shipped (wet tide band + Snell caustics inside every splat instance via the groundShade bundle). W7 shipped (double-sided surface, depth-fog overlay quad, audio low-pass). W8 shipped (`base-game-water-sim.js` shared by page and server, tick-clock controller, buoyancy/swim/crouch, crouch tick input, `waterEnabled` shared key, render clock on the same tick; `test-base-game-rooms-water.mjs` proves client and server identical through entry, swim, mid-run wave patch and exit). W9 shipped (look preset / refraction / foam / clarity in the panel, preset kept off the physics keys, saved-state and perf-capture coverage). The plan is complete: the sea has surface, reflection, refraction, depth colour, foam, caustics, an underwater view, swimming and room-synced config. Follows the terrain plan
