...
surface-contact detonation, then melee and 3.4's feedback layer (blood, one-voice impacts,
ricochet, whizz). Still open: the head multiplier only, which wants a rig on the server rather than
a capsule-top zone. Remote recoil already works via `applyAction`.
